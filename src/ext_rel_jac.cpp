                                   ...
                                   const Vector6d &rel_twist)
{
  ...
  Eigen::Matrix<double, 6, 12> L = Eigen::Matrix<double, 6, 12>::Zero(),
                               L_sim = Eigen::Matrix<double, 6, 12>::Zero();
  ...
  Eigen::Matrix<double, 14, 6> damped_a_inverse =
                                   J_a.transpose() *
                                   (J_a * J_a.transpose() +
                                    0 * damping_ * Matrix6d::Identity())
                                       .inverse(),
                               damped_sim_inverse =
                                   J_sim.transpose() *
                                   (J_sim * J_sim.transpose()).inverse();
  ...
  return damped_sim_inverse * rel_twist +
         (Eigen::Matrix<double, 14, 14>::Identity() -
          damped_sim_inverse * J_sim) *
             q_dot;
}

void ExtRelJac::getAbsoluteVelocity(const sensor_msgs::JointState &state,
                                    const Vector3d &r1, const Vector3d &r2,
...
void ExtRelJac::getRelativeVelocity(const sensor_msgs::JointState &state,
                                    const Vector3d &r1, const Vector3d &r2,
                                    Vector6d &rel_vel) const
{
  ...
  relL = 1 / ((1 - rel_alpha_) * (1 - rel_alpha_) + rel_alpha_ * rel_alpha_) *
         relL;

  rel_vel = relL * W * total_twist;
}

KDL::Frame ExtRelJac::getAbsoluteMotionFrame(const KDL::Frame &obj1,
...
KDL::Frame ExtRelJac::getRelativeMotionFrame(const KDL::Frame &obj1,
                                             const KDL::Frame &obj2) const
{
  ...
  Eigen::AngleAxisd new_obj1_ang_axis(
      (1 - rel_alpha_) * scaling * obj1_ang_axis.angle(), obj1_ang_axis.axis()),
      new_obj2_ang_axis(rel_alpha_ * scaling * obj2_ang_axis.angle(),
                        obj2_ang_axis.axis());

  Eigen::Quaterniond relative_orientation(new_obj2_ang_axis),
      new_obj1_orientation(new_obj1_ang_axis);
  ...
  relative_frame.p =
      new_obj1_rot *
      (scaling * (rel_alpha_ * obj2.p - (1 - rel_alpha_) * obj1.p));

  return relative_frame;
}

Eigen::MatrixXd ExtRelJac::computeJacobian(const sensor_msgs::JointState &state,
                                           const Vector3d &r1,
